...
# ---------------- Step 3: Build Pipeline (no leakage) ----------------
pipe = Pipeline(steps=[
...
# ---------------- Step 4: Bayesian optimization on pipeline ----------------
search_spaces = {
    "ann__alpha": Real(1e-6, 1e-2, prior="log-uniform"),
    "ann__learning_rate_init": Real(1e-4, 5e-2, prior="log-uniform")
}
...
# ---------------- Step 7: 5-fold CV on TRAIN with best hyperparams ----------------

cv_outer = KFold(n_splits=5, shuffle=True, random_state=42)
cv_scores = cross_val_score(best_model, X_train, y_train, cv=cv_outer, scoring="r2", n_jobs=-1)
...
plt.scatter(y_train, y_train_pred, c="green", label="Train",
...
# Corner metrics
plt.text(y_min, y_max - 0.03*(y_max - y_min), f"Train R²={r2_train:.3f}", color="green", fontsize=12)
plt.text(y_min, y_max - 0.08*(y_max - y_min), f"Test  R²={r2_test:.3f}",  color="orange", fontsize=12)
...
metrics_df = pd.DataFrame({
    "metric": ["R2_train", "RMSE_train", "MAE_train", "MdAE_train",
               ...
               "CV5_mean_R2_train", "CV5_std_R2_train"],
